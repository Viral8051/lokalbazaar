...
import { Search, Mic,Disc} from 'lucide-react';
...
export default function ExplorePage() {
  ...
  return (
    <Layout active="explore">
      <div className="border-b border-theme sticky top-0 bg-theme z-10" style={{padding:'12px 16px'}}>
        <h1 className="text-base font-semibold text-theme" style={{margin: '0 0 12px 0'}}>Explore</h1>
        <div className="relative flex gap-2">
          <div className="flex-1 flex items-center gap-2 bg-input border border-theme rounded-full px-3 py-2" style={{padding:'7px'}}>
            <span className="text-sm text-hint"><Search size={18}/></span>
          ...
          <button onClick={startVoice}
            ...
            <span className="text-base text-white">{listening ? <Disc size={18}/> : <Mic size={18}/>}</span>
          </button>
          {showIOSHint && (
            <div className="absolute top-12 right-0 z-50 flex flex-col items-end">
              <div className="flex items-center gap-2 bg-card border border-theme rounded-2xl px-3 py-2 shadow-xl">
                <span className="text-sm"><Mic size={18}/></span>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide" style={{padding:'12px 16px'}}>
        <div className="flex gap-2 overflow-x-auto scrollbar-hide px-4 py-3">
          <button onClick={() => setActiveCategory(null)}
            className={`flex-shrink-0 rounded-full text-xs font-medium transition-colors border ${
          ...
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(activeCategory === cat.id ? null : cat.id)}
              className={`flex-shrink-0 flex items-center gap-1.5  rounded-full text-xs font-medium transition-colors border ${
        ...
        {loading ? (
          ...
          <>
            {filteredSellers.length > 0 && (
              <div className="" style={{padding:'12px 0'}}>
                <div className="text-xs text-hint uppercase tracking-wider " style={{margin:'0 0 12px 0'}}>
                ...
                <div className="flex flex-col gap-2">
                  {filteredSellers.map(s => (
                    <div key={s.id} onClick={() => navigate(`/seller/${s.id}`)}
                      className="flex items-center gap-3 bg-card border border-theme rounded-xl cursor-pointer hover:border-accent transition-colors" style={{padding:'12px 16px'}}>
