...
function MessageStatus({ msg, userId }) {
  ...
  if (isTemp)       return <span className="text-[10px] text-white/40 ml-1" title="Sending">✓</span>
  if (msg.read)     return <span className="text-[10px] text-[#4fc3f7] ml-1" title="Seen">✓✓</span>
  if (msg.delivered)return <span className="text-[10px] text-white/70 ml-1" title="Delivered">✓✓</span>
        ...
        {loading ? (
          ...
          <div key={conv.partnerId} onClick={() => navigate(`/chat/${conv.partnerId}`)}
            className={`flex items-center gap-3 px-4 py-3 border-b border-white/5 cursor-pointer transition-colors ${
              conv.unread > 0 ? 'bg-[#f5a623]/8 hover:bg-[#f5a623]/12' : 'hover:bg-white/5'
            }`}>
            {/* Avatar + badge */}
            <div className="relative flex-shrink-0">
              ...
              {conv.unread > 0 && (
                <div className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-[#f5a623] flex items-center justify-center">
                  <span className="text-[9px] text-white font-bold leading-none">{conv.unread > 9 ? '9+' : conv.unread}</span>
                </div>
              )}
            </div>
            {/* Text */}
            <div className="flex-1 min-w-0">
              ...
              <div className={`text-xs truncate mt-0.5 ${conv.unread > 0 ? 'text-white/70 font-medium' : 'text-white/40'}`}>
                {conv.lastSenderId === user.id ? <span className="text-white/30">✓ </span> : null}{conv.lastMsg}
              </div>
            </div>
            {/* Time */}
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              <span className={`text-[10px] ${conv.unread > 0 ? 'text-[#f5a623] font-medium' : 'text-white/30'}`}>
                {timeAgo(conv.time)}
              </span>
              {conv.unread > 0 && <div className="w-2 h-2 rounded-full bg-[#f5a623]"/>}
            </div>
          </div>
        ))}
      </div>
    ...
    <Layout active="chat">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 sticky top-0 bg-[#0f0a1e] z-10">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-white/40">{partner?.role === 'seller' ? '🏪 Seller' : '🛒 Buyer'}</div>
        </div>
        {partner?.role === 'seller' && (
            ...
            <div key={msg.id}>
              {showDate && (
                <div className="flex items-center gap-2 my-3">
                  ...
                  <span className="text-[10px] text-white/30 px-2">{dateLabel(msg.created_at)}</span>
                  <div className="flex-1 h-px bg-white/10"/>
                </div>
              )}
              <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} ${showName ? 'mt-3' : 'mt-0.5'}`}>
                ...
                <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${
                  isMine ? 'bg-[#f5a623] text-white rounded-br-sm' : 'bg-white/10 text-white rounded-bl-sm'
                }`}>
                  {msg.text}
                </div>
                {isLastInGrp && (
